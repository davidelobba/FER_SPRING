...
def main(args):
    ...
    sample_test, _ = split_dataset(path=config["dataset"]["path"], perc=config["dataset"]["split_percentage"],path_audio=config["dataset"]["path_audio"])
    audio =False 
    if config["dataset"]["path_audio"] is not None:
        audio =True 
    dataset_test = RAVDESS_LANDMARK(config["dataset"]["path"], samples=sample_test, min_frames=config["dataset"]["min_frames"],n_mels=config["dataset"]["n_mels"],test=False, audio=audio,audio_only=config["training"]["audio_only"],zero_start=config["dataset"]["zero_start"],  contrastive=config["training"]["contrastive"],  mixmatch=config["training"]["augmented"], random_aug=config["training"]["random_aug"])        
    ...
    for e in range(epochs):
        ...
        with torch.no_grad():                
            for _, batch in enumerate(tqdm(loader_test)) :
                ...
                logits_1 = linear(video_feat)
                
                video_feat = vf_q2.detach()
                logits_2 = linear(video_feat)
